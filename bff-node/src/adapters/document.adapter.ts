...
export class DocumentAdapter {
  ...
  private readonly apiKey =
    process.env.API_KEY_SECRET || "mi_clave_secreta_super_segura_123";
  ...
  processDocument = async (id: string, userId: string) => {
    try {
      const response = await this.client.post(
      ...
      console.error(
        `Error disparando proceso para ${id}:`,
        error.response?.data || error.message,
      );
      throw new Error(
        `No se pudo iniciar el procesamiento del documento ${error.response?.data || error.message}`,
      );
    }
  };
}
